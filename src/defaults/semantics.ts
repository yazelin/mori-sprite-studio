...
export const DEFAULT_STATE_SEMANTICS: Record<StateName, string> = {
  ...
  walking:
    'NEUTRAL FULL-BODY STANDING REFERENCE for walking animation. Pose: standing in SIDE-PROFILE or THREE-QUARTER VIEW FACING RIGHT (the character\'s body faces toward the right edge of the frame; this is non-negotiable — DO NOT draw the character facing front or facing left, since the desktop engine mirrors via scaleX(-1) for left-direction movement so the canonical design must face RIGHT). BOTH FEET PLANTED on the ground, side by side in a neutral stance (left foot and right foot both visible, no one foot ahead of the other). Legs straight, hands relaxed at sides, body upright. ENTIRE body visible from top of hair to bottom of shoes — head not cropped, feet not cropped, both feet fully shown. Cheerful determined expression, eyes looking toward the right (the direction of travel). NO walking pose (do not lift either leg, do not show mid-stride). NO held items. This static is the IDENTITY + PROPORTIONS reference for walking — the actual walking gait cycle is rendered later in 16 separate frames.',
